...
class Yolo:
    ...
    def show_boxes(self, image, boxes, box_classes, box_scores, file_name):
        ...
        for i, box in enumerate(boxes):
            ...
            # Draw blue rectangle (BGR: blue=255,0,0), thickness 2
            cv2.rectangle(image, (x1, y1), (x2, y2), (255, 0, 0), 2)

            # Build label: class name + score rounded to 2 decimal places
            class_name = self.class_names[box_classes[i]]
            score = round(float(box_scores[i]), 2)
            label = "{} {:.2f}".format(class_name, score)

            # Draw label in red (BGR: 0,0,255), 5 pixels above top-left
            cv2.putText(
                image,
                label,
                (x1, y1 - 5),
        ...
        # If 's' key pressed, save image to detections/
        if key == ord('s'):
